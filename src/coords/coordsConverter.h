...
LineSegment CoordsConverter::createLineSegment(const int startWaypointIndex,
		const int endWaypointIndex) const {
...
const CoordSys CoordsConverter::createCoordSys(const Point& point,
		const int startWaypointIndex, const int endWaypointIndex) const {
...
Frenet CoordsConverter::getFrenet(const Point& point) const {
	...
	const CoordSys coordSysPrev2Closest = createCoordSys(point,
			adaptWaypointIndex(closestIndex - 1), closestIndex);
	const CoordSys coordSysClosest2Next = createCoordSys(point, closestIndex,
			adaptWaypointIndex(closestIndex + 1));
...
LineSegment CoordsConverter::getLineSegmentContaining(
	...
	LineSegment lineSegment = map_waypoints.getLineSegment(startIndex,
			adaptWaypointIndex(startIndex + 1));
	return lineSegment;
}
...
Frenet CoordsConverter::createFrenetVectorFromStart2End(const Point& start,
		const Point& end) const {
